...
const publishedSockets = globalThis[kSockets]
...
const sockets = publishedSockets ??
  (legacyValue instanceof Map ? legacyValue : new Map())

// Frozen globals cannot accept a new Symbol slot. The module-local Map remains
// usable by this copy even when it cannot be published for other copies.
if (publishedSockets == null && Object.isExtensible(globalThis)) {
  globalThis[kSockets] = sockets
}
...
if (legacyDescriptor?.configurable !== false &&
    (legacyDescriptor !== undefined || Object.isExtensible(globalThis))) {
