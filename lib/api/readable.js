...
const kDumpLimit = Symbol('kDumpLimit')

const noop = () => {}

/**
 * 'data' listener installed by dump(); shared so a dump does not allocate a
 * closure per call.
 *
 * @this {BodyReadable}
 */
function onDumpData () {
  if (this[kBytesRead] > this[kDumpLimit]) {
    this.destroy()
  }
}
...
class BodyReadable extends Readable {
  ...
  _destroy (err, callback) {
    ...
    // Defer destruction when the stream is unused or a body-mixin setup
    // microtask is pending. This lets consumers install their error and close
    // listeners before destruction events are emitted.
    if (!this[kUsed] || this[kConsumePending]) {
   ...
   */
  dump (opts) {
    // Not an async function: that would wrap the promise below in a second
    // one. Errors before it is created are still reported by rejection.
    let signal
    let limit
    try {
      signal = opts?.signal
      ...
      limit = opts?.limit && Number.isFinite(opts.limit)
        ? opts.limit
        : 128 * 1024
      ...
      if (typeof signal?.throwIfAborted === 'function') {
        ...
        // AbortSignalLike also permits structural signals without this method.
    ...
    return new Promise((resolve, reject) => {
      ...
      // Only successful setup contributes a limit. Concurrent dumps share
      // onDumpData, so the smallest active limit applies.
      this[kDumpLimit] = Math.min(this[kDumpLimit], limit)
