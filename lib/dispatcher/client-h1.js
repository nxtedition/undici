...
const HEADER_SLICE_SIZE = 64 * 1024
const FAST_BODY_MIN_SIZE = 256 * 1024
const FAST_BODY_MIN_DIGITS = String(FAST_BODY_MIN_SIZE).length
...
/**
 * Parse a single Content-Length field value for use as a JavaScript framing
 * boundary. Keep the normal llhttp path for duplicate, malformed, or unsafe
 * integer values so the fast path cannot weaken framing validation.
 *
 * @param {string} value
 * @returns {number}
 */
function parseContentLength (value) {
  let i = 0
  while (i < value.length && (value.charCodeAt(i) === 0x20 || value.charCodeAt(i) === 0x09)) {
    i++
  }
  ...
  for (; i < value.length; i++) {
    ...
    if (length > Math.floor((Number.MAX_SAFE_INTEGER - digit) / 10)) {
      return -1
    }
  ...
  return i === value.length ? length : -1
}
...
class Parser {
  ...
  constructor (client, socket, { exports }) {
    ...
    this.keepAlive = ''
    this.contentLength = ''
    this.contentLengthHeaders = 0
    this.fastBodyLength = -1
    this.connection = ''
  ...
  resume () {
    ...
    if (this.fastBody) {
      ...
      this.paused = false
      if (this.bytesRead === this.fastBodyLength) {
  ...
  execute (chunk) {
    ...
    // Preserve the fork's one-shot llhttp hot path for normal socket reads.
    // The sliced loop below is only needed for unusually large reads while
    // waiting for response headers.
    if (chunk.length <= HEADER_SLICE_SIZE) {
    ...
    try {
      ...
      do {
        ...
        new Uint8Array(llhttp.memory.buffer, currentBufferPtr, currentBufferSize).set(slice)
  ...
  executeFastBody (chunk) {
    ...
    const remaining = this.fastBodyLength - this.bytesRead
    assert(remaining > 0)
    ...
    const paused = request.onData(body) === false

    if (request.aborted || socket.destroyed) {
      return
    }
  ...
  completeFastBody () {
    ...
    assert(this.bytesRead === this.fastBodyLength)
  ...
  finish () {
    ...
    if (this.paused) {
      if (this.fastBody && this.bytesRead === this.fastBodyLength) {
        ...
        do {
          ...
          // Preserve the fork's RST handling for bodies still owned by llhttp:
          // executing an empty chunk on a destroyed socket can consume the
          // incomplete state and hide a Content-Length mismatch from finish().
          if (!this.fastBody && this.socket.destroyed && data.length === 0) {
            const hasCompleteContentLength =
              this.contentLength !== '' &&
              this.bytesRead === parseInt(this.contentLength, 10)
            if (!hasCompleteContentLength) {
              break
            }
          }
          this.execute(data)
          ...
          this.paused &&
          data.length > 0 &&
          !(this.fastBody && this.bytesRead === this.fastBodyLength)
        )
  ...
  onHeadersComplete (statusCode, upgrade, shouldKeepAlive) {
    ...
    const fastBodyLength = this.contentLengthHeaders === 1 && this.contentLength.length >= FAST_BODY_MIN_DIGITS
      ? parseContentLength(this.contentLength)
      : -1

    if (
      fastBodyLength >= FAST_BODY_MIN_SIZE &&
      ...
      this.fastBodyLength = fastBodyLength
      this.fastBody = true
      return constants.ERROR.PAUSED
    }
...
function onHttpSocketError (err) {
  ...
  if (
    err.code === 'ECONNRESET' &&
    parser.statusCode &&
    (!parser.shouldKeepAlive || parser.paused || parser.fastBody)
  ) {
...
function onHttpSocketEnd () {
  ...
  if (parser.statusCode && (!parser.shouldKeepAlive || parser.paused || parser.fastBody)) {
    const parserErr = parser.finish()
    if (parserErr) {
...
function onHttpSocketClose () {
  ...
  if (parser) {
    if (!this[kError] && parser.statusCode && (!parser.shouldKeepAlive || parser.fastBody)) {
