...
class Parser {
  ...
  onHeaderValue (buf) {
    ...
    // These values are only compared against ASCII, so decode them as latin1
    // and skip Buffer#toString's encoding dispatch.
    const key = this.headers[len - 2]
    if (key.length === 10) {
      const headerName = util.bufferToLowerCasedHeaderName(key)
      if (headerName === 'keep-alive') {
        this.keepAlive += buf.latin1Slice(0, buf.length)
      } else if (headerName === 'connection') {
        this.connection += buf.latin1Slice(0, buf.length)
      }
    } else if (key.length === 14 && util.bufferToLowerCasedHeaderName(key) === 'content-length') {
      this.contentLength += buf.latin1Slice(0, buf.length)
    }
