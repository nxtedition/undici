  ...
  return await WebAssembly.instantiate(mod, {
    env: {
      ...
      wasm_on_header_field: (p, at, len) => {
        assert(currentParser.ptr === p)
        // Header names and values are handed on as latin1 strings: parseHeaders
        // decodes them that way anyway, so a Buffer view per field line would
        // only be extra garbage.
        const start = at - currentBufferPtr
        return currentParser.onHeaderField(currentBufferRef.latin1Slice(start, start + len))
      },
      /**
       * @param {number} p
       * @param {number} at
      ...
      wasm_on_header_value: (p, at, len) => {
        assert(currentParser.ptr === p)
        const start = at - currentBufferPtr
        return currentParser.onHeaderValue(currentBufferRef.latin1Slice(start, start + len))
      },
      /**
       * @param {number} p
...
class Parser {
  ...
  /**
   * @param {string} str latin1-decoded fragment of a header name
   * @returns {number}
   */
  onHeaderField (str) {
  ...
  onHeaderValue (str) {
    ...
    if (key.length === 10) {
      if (isHeaderName(key, 'keep-alive')) {
        this.keepAlive += str
      } else if (isHeaderName(key, 'connection')) {
        this.connection += str
      }
    } else if (key.length === 14 && isHeaderName(key, 'content-length')) {
...
/**
 * Compares a header name with a lowercase ASCII name of the same length,
 * ignoring case, without allocating a lowercased copy.
 *
 * @param {string} key
 * @param {string} name
 * @returns {boolean}
 */
function isHeaderName (key, name) {
