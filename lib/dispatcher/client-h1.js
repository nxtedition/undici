...
class Parser {
  ...
  setTimeout (delay, type) {
    ...
    if (
      ...
      if (typeof this.timeout.refresh === 'function') {
        this.timeout.refresh()
      } else {
        // FastTimer handles always expose refresh(). Native timer shims may
        // return numeric handles, so replace those timers to refresh them.
        timers.clearTimeout(this.timeout)
        this.timeout = setTimeout(onParserTimeout, delay, this.timeoutWeakRef)
        if (typeof this.timeout?.unref === 'function') {
          this.timeout.unref()
  ...
  finish () {
    ...
    if (this.paused) {
      ...
      do {
        ...
        // !paused), but we must NOT push an empty buffer through execute(): on a
        // destroyed socket that consumes the half-parsed body state and makes
        // llhttp_finish() report OK, silently dropping a truncated
        // Content-Length body. Break straight to llhttp_finish() instead, which
        // then detects the incomplete message and surfaces a
        // ResponseContentLengthMismatchError, matching the clean-FIN path.
        if (this.socket.destroyed && data.length === 0) {
          break
        }
        this.execute(data)
...
function writeH1 (client, request) {
  ...
  if (util.isBlobLike(body) && request.contentType == null && body.type) {
  ...
  let blobStream
  let useBlobStream = false
  if (util.isBlobLike(body) && typeof body.stream === 'function') {
    try {
      blobStream = body.stream()
      useBlobStream = true
    } catch (err) {
      // No request bytes or socket state have been changed yet. Reject this
      // request without tearing down an otherwise reusable connection.
      util.errorRequest(client, request, err)
      return false
    }
  }
  ...
  if (socket[kTOS] !== request.typeOfService) {
    if (socket.setTypeOfService) {
      socket.setTypeOfService(request.typeOfService)
    }
    socket[kTOS] = request.typeOfService
  }
