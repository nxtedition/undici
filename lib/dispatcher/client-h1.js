...
// constants.ERROR maps names to codes and has no reverse entries (it stopped
// being a TypeScript enum), so build the code -> name lookup used to name an
// HTTPParserError.
const ERROR_NAMES = Object.fromEntries(Object.entries(constants.ERROR).map(([name, code]) => [code, name]))
const EMPTY_BUF = Buffer.alloc(0)
// Lowercased well-known header names, indexed as the WASM glue numbers them:
// 1-based, in wellknownHeaderNames order, with 0 for any other name. The
// strings come from headerNameLowerCasedRecord, whose keys they already are,
// so reusing one allocates nothing and keys the header map with an
// internalized string.
const WELLKNOWN_HEADER_NAMES = ['', ...wellknownHeaderNames.map((name) => headerNameLowerCasedRecord[name])]
  ...
  return await WebAssembly.instantiate(mod, {
    env: {
      ...
      wasm_on_status: (p, at, len) => {
        // The reason phrase is never surfaced to handlers, so don't decode it.
       ...
       * @param {number} at
       * @param {number} len
       * @param {number} wellknown index into WELLKNOWN_HEADER_NAMES, or 0
       * @returns {number}
       */
      wasm_on_header_field: (p, at, len, wellknown) => {
        assert(currentParser.ptr === p)
        if (wellknown !== 0) {
          return currentParser.onHeaderField(WELLKNOWN_HEADER_NAMES[wellknown])
        }
        // The WASM glue lowercases the name in place, so it is read back from
        // linear memory rather than from the chunk that was copied there.
        // Names and values are handed on as latin1 strings, which is what the
        // header map stores, so a Buffer view per field line would only be
        // extra garbage.
        const start = at - currentBufferPtr
        return currentParser.onHeaderField(currentBuffer.latin1Slice(start, start + len))
      },
      /**
       * @param {number} p
       * @param {number} at
      ...
      wasm_on_header_value: (p, at, len) => {
        assert(currentParser.ptr === p)
        const start = at - currentBufferPtr
        return currentParser.onHeaderValue(currentBuffer.latin1Slice(start, start + len))
      },
      /**
       * @param {number} p
...
// The part of a field line the parser last delivered. llhttp may split a name
// or a value across callbacks, so a field line is added to the header map once
// the next name starts or the field section ends.
const HEADER_NONE = 0
...
class Parser {
  ...
  finish () {
    ...
    if (this.paused) {
      ...
      do {
        ...
        // If the peer reset the connection (RST -> ECONNRESET) the socket is
        // already destroyed and there is nothing left to read. The parser was
        // just resumed above (required before llhttp_finish(), which asserts
        // !paused). Pushing an empty buffer through execute() completes a body
        // that paused on its final byte, but on a truncated body it consumes the
        // half-parsed state and makes llhttp_finish() report OK. Only flush an
        // empty destroyed socket when the declared Content-Length was received;
        // otherwise break straight to llhttp_finish(), which detects the
        // incomplete message and surfaces a ResponseContentLengthMismatchError.
        const hasCompleteContentLength =
          this.contentLength !== '' &&
          this.bytesRead === parseInt(this.contentLength, 10)
        if (this.socket.destroyed && data.length === 0 && !hasCompleteContentLength) {
          break
        }
        this.execute(data)
  ...
  createError (ret, data) {
    ...
    return new HTTPParserError(message, ERROR_NAMES[ret], data)
  }
  ...
  /**
   * @param {string} str latin1-decoded, lowercased fragment of a header name
   * @returns {number}
   */
  onHeaderField (str) {
    if (this.headerState === HEADER_FIELD) {
      this.headerName += str
    } else {
      if (this.headerState === HEADER_VALUE) {
        this.addHeader()
      }
      this.headerName = str
      this.headerState = HEADER_FIELD
    }

    this.trackHeader(str.length)

    return 0
  }

  /**
   * @param {string} str latin1-decoded fragment of a header value
   * @returns {number}
   */
  onHeaderValue (str) {
    if (this.headerState === HEADER_VALUE) {
    ...
    } else if (name.length === 14 && name === 'content-length') {
  ...
  /**
   * Adds the completed field line to the header map. The name is already
   * lowercased, so two spellings of one name share an entry, and a repeated
   * field line turns the entry into an array.
   *
   ...
   */
  addHeader () {
    const { headers, headerName: name, headerValue: value } = this
   ...
   */
  takeHeaders () {
    assert(this.headerState !== HEADER_FIELD)
  ...
  onMessageComplete () {
    ...
    if (
      ...
      (
        ...
        (
          shouldKeepAlive &&
          (contentLength === '' || bytesRead !== parseInt(contentLength, 10))
        )
      )
    ) {
      return -1
    ...
    if (request.method !== 'HEAD' && statusCode !== 304 && contentLength && bytesRead !== parseInt(contentLength, 10)) {
