...
test('pre-aborted request does not connect', async (t) => {
  ...
  assert.notStrictEqual(result, timeout, 'pre-aborted request did not reject promptly')
  assert.strictEqual(result, reason)
  assert.strictEqual(connectCalls, 0)
})

test('abort while connecting rejects before the connector settles', async (t) => {
  let connectCallback
  let connectCalls = 0
  let callbackCalls = 0
...
test('queued abort uses one user-signal listener', async (t) => {
  const client = new Client('http://localhost', {
    connect () {}
  })
  t.after(() => client.destroy())
...
test('shared connection attempt is cancelled only after every request aborts', async (t) => {
  const attempts = []
  let connectionErrors = 0
...
test('a late socket from a cancelled attempt cannot replace a newer attempt', async (t) => {
  const attempts = []
  let connectEvents = 0
...
test('the built-in connector destroys a pending socket when its attempt aborts', async (t) => {
...
test('intentional preconnect is not cancelled without a queued request', async (t) => {
  let attemptSignal
  const client = new Client('http://localhost', {
    connect (opts) {
  ...
  assert.strictEqual(attemptSignal.aborted, false)

  await client.destroy()
  assert.strictEqual(attemptSignal.aborted, true)
})

test('cancelling a Pool attempt does not evict the client or block close', async (t) => {
  const attempts = []
  let connectionErrors = 0
