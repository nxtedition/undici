...
/* undici: field names are case-insensitive, so they reach JS lowercased and
 * can key a header map as-is, together with their 1-based index in
 * wellknownHeaderNames (0 for any other name) so JS can reuse a preallocated
 * string instead of decoding one. A span cut at the end of the input can match
 * a shorter well-known name; JS appends the next fragment to that string just
 * as it would to a decoded one.
 *
 * Only ASCII 'A'-'Z' change; other bytes, obs-text included, are left alone.
 * The span is lowercased in place: it lies in the input buffer JS copied into
 * linear memory, and llhttp has already consumed it, so the parser never sees
 * the change. Every load and store stays inside the span. Lowercasing is
 * idempotent, so a tail overlaps the previous block instead of falling back to
 * a scalar loop. */
#if defined(__wasm_simd128__)
